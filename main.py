...
async def get_countries():
    america_countries = requests.get(
...
async def insert_addresses(conn):
    ...
    logradouros = {
        logradouro["Ds_Logradouro"].strip(): logradouro["Cd_Logradouro"]
        for logradouro in logradouros
    }
    neighborhoods = await get_all_data(conn, "Bairro")
    neighborhoods = {
        neighborhood["Nm_Bairro"].strip(): neighborhood["Cd_Bairro"]
        for neighborhood in neighborhoods
    }
    logradouros_types = await get_all_data(conn, "TipoLogradouro")
    logradouros_types = {
        logradouro_type["Ds_TipoLogradouro"].strip(): logradouro_type[
            "Cd_TipoLogradouro"
        ]
        for logradouro_type in logradouros_types
    }
    complement_type = await get_all_data(conn, "TipoComplemento")
    complement_type = {
        complement["Ds_TipoComplemento"].strip(): complement["Cd_TipoComplemento"]
        for complement in complement_type
    }
    ...
    for address in response:
        ...
        address["TipoComplemento"] = (
            address["Complemento"].split()[0] if address["Complemento"] else ""
        )
        address["Complemento"] = (
            " ".join(address["Complemento"].split()[1:])
        ...
        if address["Logradouro"] not in logradouros.keys():
            await insert_data(
                conn, "Logradouro", {"Ds_Logradouro": address["Logradouro"]}
            )
            logradouros = await get_all_data(conn, "Logradouro")
            logradouros = {
                logradouro["Ds_Logradouro"].strip(): logradouro["Cd_Logradouro"]
        ...
        if address["TipoLogradouro"] not in logradouros_types.keys():
            await insert_data(
                conn, "TipoLogradouro", {"Ds_TipoLogradouro": address["TipoLogradouro"]}
            )
            logradouros_types = await get_all_data(conn, "TipoLogradouro")
            logradouros_types = {
                logradouro_type["Ds_TipoLogradouro"].strip(): logradouro_type[
        ...
        if address["TipoComplemento"] not in complement_type.keys():
            await insert_data(
                conn,
                "TipoComplemento",
                {"Ds_TipoComplemento": address["TipoComplemento"]},
            )
            complement_type = await get_all_data(conn, "TipoComplemento")
            complement_type = {
                complement["Ds_TipoComplemento"].strip(): complement[
                    "Cd_TipoComplemento"
                ]
                for complement in complement_type
            }
    ...
    logging.warning(
        f"Warning! Total neighborhoods not found: {total_neighborhood_not_found}"
    )
...
async def insert_addresses_list(conn):
    ...
    address_types = {
        address_type["Ds_TipoEndereco"].strip(): address_type["Cd_TipoEndereco"]
    ...
    factories_persons = [
        await get_by_id(conn, "PessoaJuridica", factory_id) for factory_id in factories
    ]
    factories_persons = [
        factory_person["Cd_Pessoa"] for factory_person in factories_persons
    ]
    ...
    vaccine_centers = [
        vaccine_center["Cd_PessoaJuridica"] for vaccine_center in vaccine_centers
    ]
    vaccine_centers_persons = [
        await get_by_id(conn, "PessoaJuridica", vaccine_center_id)
        for vaccine_center_id in vaccine_centers
    ]
    vaccine_centers_persons = [
        vaccine_center_person["Cd_Pessoa"]
        for vaccine_center_person in vaccine_centers_persons
    ]
    ...
    patients_persons = [
        await get_by_id(conn, "PessoaFisica", patient_id) for patient_id in patients
    ]
    patients_persons = [
        patient_person["Cd_Pessoa"] for patient_person in patients_persons
    ]
...
async def insert_ships(conn):
    ...
    vaccine_centers = await get_all_data(conn, "CentroVacinacao")
    vaccine_centers = [
        vaccine_center["Cd_CentroVacinacao"] for vaccine_center in vaccine_centers
    ]
    vaccine_types = await get_all_data(conn, "TipoVacina")
    vaccine_types = {
        vaccine_type["Cd_TipoVacina"]: vaccine_type["Pz_Validade"]
        for vaccine_type in vaccine_types
    }
    ...
    for _ in range(150):
        ...
        expiration_date = manufactured_date + timedelta(
            days=vaccine_types[vaccine_type]
        )
...
async def insert_shifts(conn):
    ...
    vaccination_centers = [
        vaccination_center["Cd_CentroVacinacao"]
...
async def insert_ampoules(conn):
    ...
    for ship in ships.keys():
        for _ in range(int(ships[ship][0])):
            ...
            ampoule_info["Dt_Abertura"] = fake.date_time_between(
                start_date=ships[ship][1], end_date=ships[ship][2]
            )
...
async def insert_vaccines(conn):
    ...
    shifts = {
        shift["Cd_Plantao"]: [
            shift["Cd_Funcionario"],
            shift["Cd_CentroVacinacao"],
            shift["Dt_Inicio"],
            shift["Dt_Termino"],
        ]
        for shift in shifts
    ...
    ampoules = {
        ampoule["Cd_Ampola"]: [ampoule["Cd_Lote"], ampoule["Dt_Abertura"]]
    ...
    for _ in range(1000):
        ...
        vaccine_date = fake.date_time_between(
            start_date=shifts[shift][2], end_date=shifts[shift][3]
        )
...
async def generate_patient_with_all_vaccines(conn):
    ...
    ships = [
        await get_by_id(conn, "Lote", ampoules[ampoule_id][0])
        for ampoule_id in ampoules.keys()
    ]
    ships = {
        ship["Cd_Lote"]: [ship["Cd_TipoVacina"], ship["Cd_CentroVacinacao"]]
        for ship in ships
    }
    vaccine_types = await get_all_data(conn, "TipoVacina")
    ...
    for type in vaccine_types:
        ...
        for shift in shifts.keys():
            if (
                shifts[shift][1] == vaccine_center_id
                and shifts[shift][2] > ampoule_open_date
            ):
                shift_id = shift
        ...
        vaccine_info["Dt_Vacinacao"] = fake.date_time_between(
            start_date=shift_start, end_date=shift_end
        )
        await insert_data(conn, "Vacinacao", vaccine_info)
...
async def generate_patient_with_all_factories(conn):
    ...
    for factory_id in factories:
        ...
        for shift_id, shift_data in shifts.items():
            ...
            if (
                shift_data["center_id"] == vaccine_center_for_ship
                and shift_data["start_time"] >= target_ampoule_open_date
            ):
        ...
        possible_vaccination_start_date = max(
            target_shift_start, target_ampoule_open_date
        )
        ...
        vaccine_info["Dt_Vacinacao"] = fake.date_time_between(
            start_date=possible_vaccination_start_date, end_date=target_shift_end
        )
